...
public class MySql {
    ...
    public static Compte getAccountByName(String name) {
        ...
        try {
            ...
            String query = "SELECT vip, m_password, m_reg, banned, banned_time, pass, saltKey, logged, pseudo, question, guid, level, accessLevel FROM accounts WHERE account LIKE ? LIMIT 1;";
            PreparedStatement ps = co.prepareStatement(query);
            ps.setString(1, name);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                compte = new Compte(rs.getString("pass"), rs.getString("saltKey"), rs.getInt("m_password"), rs.getInt("banned"), rs.getInt("logged"), rs.getString("pseudo"), rs.getString("question"), rs.getInt("guid"), name, rs.getInt("level"), rs.getInt("banned_time"), rs.getInt("vip"), rs.getInt("m_reg"), rs.getInt("accessLevel"));
            }
            rs.close();
            ps.close();
            co.close();
